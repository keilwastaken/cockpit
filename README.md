...
Run `/cockpit setup` for the onboarding wizard. Setup is simplified to two model choices: the **hands model** inherited by implementation workers (`instant`, `fast`, `normal`) and the **reasoning model** inherited by research/planning/review workers (`research`, `planner`, `reviewer`). Recommended: local model for hands, latest cloud reasoning model for reasoning. Thinking is forced per flow: instant off, research minimal, fast low, normal medium, planner xhigh, reviewer high. Strict mode is recommended so the main chat stays the Oracle and delegates perform code mutation.
